...
class CommonFeatureWeighting(BaseItemSimilarityMatrixRecommender):

    def __init__(self, URM, ICM, W_sparse_CF):
        super(CommonFeatureWeighting, self).__init__(URM)

        if (URM.shape[1] != ICM.shape[0]):
            raise ValueError(
                "Number of items not consistent. URM contains {} but ICM contains {}".format(URM.shape[1],
                                                                                             ICM.shape[0]))

        if (W_sparse_CF.shape[0] != W_sparse_CF.shape[1]):
            raise ValueError(
                "Items imilarity matrix is not square: rows are {}, columns are {}".format(W_sparse_CF.shape[0],
                                                                                           W_sparse_CF.shape[1]))

        if (W_sparse_CF.shape[0] != ICM.shape[0]):
            raise ValueError(
                "Number of items not consistent. S_matrix contains {} but ICM contains {}".format(W_sparse_CF.shape[0],
                                                                                                  ICM.shape[0]))

        self.URM = URM
        self.W_sparse_CF = check_matrix(W_sparse_CF, 'csr')
    ...
    def recommend(self, user_id, at=None, exclude_seen=True):
        ...
        user_profile = self.URM.train[user_id]
    ...
    # Method derived from the original implementation
    def _generateTrainData_low_ram(self):
        ...
        self._writeLog(self.RECOMMENDER_NAME + ": Collaborative S density: {:.2E}, nonzero cells {}".format(
            self.W_sparse_CF.nnz/self.W_sparse_CF.shape[0]**2, self.W_sparse_CF.nnz))
        ...
        for row_index in range(self.n_items):
            ...
            start_pos_target = self.W_sparse_CF.indptr[row_index]
            end_pos_target = self.W_sparse_CF.indptr[row_index+1]

            target_coordinates = self.W_sparse_CF.indices[start_pos_target:end_pos_target]
            ...
            for index in range(len(is_common)):
                ...
                if is_common[index]:
                    ...
                    new_data_value = self.W_sparse_CF[row_index, col_index]
        ...
        collaborative_nnz = self.W_sparse_CF.nnz
        collaborative_sum = sum(self.W_sparse_CF.data)
